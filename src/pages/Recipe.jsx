...
import { Tag, List, Divider, Row, Col } from "antd";
    ...
    .map(([,ingredient], index) => {
        ...
        return (
            <span className="ingredient-item" key={index}>
                ...
                <span className="ingredient-measure accent-color">{measure}</span>
            </span>
        ); 
    });
                ...
                <>
                    <div className="recipe-text-block">
                        <span className="recipe-title-block">
                            <h1 className="recipe-title"><i className="accent-color w-600">{recipe.strMeal}</i></h1>
                            <FlagDisplay countryName={recipe.strArea} />
                        </span>
                        <span className="recipe-tag-block">
                            {recipe.strTags && (
                                recipe.strTags.split(",").map((el, index) => (
                                    <Tag className="recipe-tag" key={index}>{el.trim()}</Tag>
                                )))
                            }
                    ...
                    <Row gutter={[24, 32]} justify={"center"}>
                        ...
                        <Col xs={24} md={20} lg={20} xxl={16}>
                            <div className="recipe-content-block">
                                ...
                                <ul className="recipe-algorithm-text">{recipe.strInstructions.split("\r\n\r\n").map((el, index) => (
                                    <li key={index}>{el}</li>
                                ))}</ul>
                            </div>
                        </Col>
